...
std::string StaticVariable::autoComPortName = "";		// init Get COM Port automatically.

MainFrame::MainFrame(const wxString &title)
: wxFrame(nullptr, wxID_ANY, title) {
	CreateStatusBar();
	
	wxButton *button = new wxButton(this, wxID_ANY, "Get Serial Port", wxPoint(200,200), wxSize(250,50));
	button->Bind(wxEVT_BUTTON, &MainFrame::ClickedButton, this);

}

void MainFrame::ClickedButton(wxCommandEvent &evt) {
	/*GetSerialPort getSerialPort;
	int num = getSerialPort.getCOMPort();
	wxLogStatus("Result num = %d", num);
	*/
	int num = MainFrame::getCOMPort();
	wxLogStatus("Result num = %d", num);
	
}


int MainFrame::getCOMPort() {
